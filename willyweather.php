...
/*
	Plugin Name: US Weather Widget
	Plugin URI: http://willyweather.com/
	Description: The most accurate US weather widgets, with data provided by the National Oceanic and Atmospheric Administration (NOAA). Willyweather weather widgets come in many different shapes and sizes, you can choose your own colors and select from multiple weather types such as Weather, Wind, Rain, Swell, Tides, UV, Sun and Moon.
	Author: WillyWeather
	Author URI: http://willyweather.com/
	Version: 1.5
*/
...
class WW_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			'WillyWeather',
			array(
				'description' => __('Highly detailed weather widgets with thousands of US locations','text_domain')
			)
		);
	...
	?>
		...
		<script>
			...
			jQuery(document).ready(function(){
				...
				if (0<?php echo esc_attr($id); ?>) {
					...
					jQuery.ajax({
						type: 'POST',
						url: '//www.willyweather.com/widget/plugin/get.html?id=<?php echo esc_attr($id); ?>&code=<?php echo esc_attr($code); ?>',
						dataType: 'jsonp',
						timeout: 20000,
		...
		<input type="hidden" name="widget[locations][0][id]" value="229182"><!--default-->
        <input type="hidden" name="widget[locations][0][typeId]" value="1"><!--default -->
        <input type="hidden" name="widget[locations][0][name]" value="New York"><!--default -->
        <input type="hidden" name="widget[locations][0][displayName]" value="New York, NY 10007"><!--default -->
		...
		<div class="ww-widget-location">
			<p>
				...
				<input class="widefat" type="search" name="ww-widget-location" placeholder="New York, NY 10007" autocomplete="off"><!--default-->
